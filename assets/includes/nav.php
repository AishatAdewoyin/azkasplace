<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8" />
    <meta http-equiv="X-UA-Compatible" content="IE=edge" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <link rel="stylesheet" href="assets/CSS/bootstrap.min.css" />
    <link rel="stylesheet" href="assets/fonticons/css/all.css" />
    <link rel="stylesheet" href="assets/CSS/style.css" />
    <link rel="icon" href="assets/images/AZKA favicon-modified.png" />
    <title>AzkasPlace | Eplicitly Fashionable</title>

    <!-- Google Fonts -->
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Dancing+Script:wght@500&family=Kaushan+Script&family=Metal&family=Nobile:wght@500&family=Roboto:wght@500&display=swap" rel="stylesheet">
</head>

<body>
    <!-- Navbar -->

    <section>

        <nav class="navbar navbar-expand-lg bg-light fixed-top">
            <div class="container-fluid">
                <a class="navbar-brand" href="index.php">
                    <img src="assets/images/AZKA white bgcropped.png" alt="" class="navbrand" />
                </a>
                <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
                    <span class="navbar-toggler-icon"></span>
                </button>

                <div class="collapse navbar-collapse nav-buttons" id="navbarSupportedContent">
                    <ul class="navbar-nav navlinks me-auto mb-2 mb-lg-0">
                        <li class="nav-item">
                            <a class="nav-link links" href="products.php">Products</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link links" href="affiliate.php">Affiliate</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link links" href="blog.php">Blog</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link links" href="about.php">About</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link links" href="contact.php">Contact</a>
                        </li>
                        <li class="navicons">
                            <a href="cart" class="text-light iconli">
                                <i class="fa-solid fa-basket-shopping"></i>
                            </a>
                            <a href="login.php" class="text-light iconli">
                                <i class="fa-solid fa-user"></i>
                            </a>
                        </li>
                    </ul>
                </div>
            </div>
        </nav>
    </section>

</body>